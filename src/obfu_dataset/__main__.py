...
def get_mix2_command(root_path, proj, obf_level, seed, script_ida, output_path) -> list[str]:
    # In order to have the complete list of functions that are available for obfuscation, need to rely on -O0 binary
    src_dir = root_path / proj.value / "sources"
    ...
    return cmd
def get_mix1_command(root_path, proj, obf_level, seed, script_ida, output_path) -> list[str]:
    # In order to have the complete list of functions that are available for obfuscation, need to rely on -O0 binary
    src_dir = root_path / proj.value / "sources"
    plain_binary = [f for f in src_dir.iterdir() if (f.suffix == '.exe') and ('O0' in f.name)][0]
    plain_src = [f for f in src_dir.iterdir() if (f.suffix == '.c')][0]
    ...
    with open(symbols_path, 'r') as file:
        function_names = json.load(file)
    function_list = [v for v in function_names.values()]
    random.Random(seed).shuffle(function_list)
...
def get_merge_command(root, proj, obf_level, seed, script_ida, output_path):
    # In order to have the complete list of functions that are available for obfuscation, need to rely on -O0 binary
    src_dir = Path(root) / (proj + "/sources/")
    plain_binary = [f for f in src_dir.iterdir() if (f.suffix == '.exe') and ('O0' in f.name)][0]
    plain_src = [f for f in src_dir.iterdir() if (f.suffix == '.c')][0]
    symbols_path = plain_binary.parents[0] / (plain_binary.name.replace('.exe', '.json'))
    if not symbols_path.exists():
        os.environ["IDA_PATH"]=shutil.which("ida")
        ida=IDA(plain_binary, script_ida, [])
        ida.start()
    ...
    with open(symbols_path, 'r') as file:
        function_names = json.load(file)
    functions_list = [v for v in function_names.values()]
    # Cleaning candidate functions
    error_functions_zlib = {'___errno_location', 'vsnprintf',  'printf',  'sub_401020', 'strerror', '__libc_start_main', '_putchar','_puts', 'malloc', '_close', '_lseek', 'puts', 'lseek', 'close', 'snprintf', 'write', 'putchar', 'strlen', 'open', 'read', 'memcpy', 'memset', 'free', 'memchr'}
    error_functions_lz4 = {'deregister_tm_clones', 'register_tm_clones', '__do_global_dtors_aux', 'frame_dummy', '_start', '_init', '_fini', 'zcalloc', 'zcfree', 'main', 'gzprintf'}
    error_functions_minilua = {'_fini','__do_global_dtors_aux', 'register_tm_clones','frame_dummy', 'LZ4F_compressBlock', 'LZ4F_compressBlockHC_continue', 'main','LZ4F_doNotCompressBlock','LZ4F_compressBlock_continue', 'LZ4F_compressBlockHC', 'deregister_tm_clones', '_start', '_init'}
    ...
    error_functions = error_functions_zlib.union(error_functions_lz4.union(error_functions_minilua).union(error_functions_sqlite))
    candidates_to_merge = list({f for f in functions_list if ('i$nit' not in f) and (not f.startswith('_'))} - error_functions)
    ...
    cmd.append('--out=' + str(output_path))
...
def gen_ollvm_obfuscated(root, proj, obf, obf_level, seed, script_ida):
    ...
    #All minilua functions are included inside a .h file. Need to adapt it compared to other projects
    if proj.value in ['minilua']:
        plain_h = [f for f in src_dir.iterdir() if (f.suffix == '.h') ][0]
        with open(minilua_h, 'r') as out :
            readlines = out.readlines()
            
        for func in candidate_functions:
            candidates = [(i, line) for (i, line) in enumerate(readlines) if (' '+func+' (' in line) and (';' not in line) and (':' not in line) and ('\\' not in line) and (' '+func+'()' not in line) and ('if ' not in line) and ('return ' not in line) and ('=' not in line) and ('/* '+func not in line)]
            ...
            for c in candidates:
                first_idx, line = c[0], c[1]
                signature = line.split(func)[0]
                if signature.startswith('static') or signature.startswith('LUA_API') or signature.startswith('l_noret') or signature.startswith('LUALIB_API'):
                    args = line.split(func)[1].replace('{', '')
                    end_args_cpt = 1
                    while ')' not in args: #déclaration tient sur plusieurs lignes
        ...
        obf_basename_h = proj.value + "_ollvm_clang_x64_" + obf.value + '_' + str(obf_level) + '_' + str(seed) + '.h'
        with open(obf_dir / obf_basename_h, 'w') as out:
            for line in readlines : 
                out.write(line)
        
        obf_basename_c = proj.value + "_ollvm_clang_x64_" + obf.value + '_' + str(obf_level) + '_' + str(seed) + '.c'
        with open(plain_src, 'r') as out:
            readlines = out.readlines()
        with open(obf_dir / obf_basename_c, 'w') as out:
            for line in main_readlines : 
                if 'minilua.h' in line : 
                    out.write('#include "' + str(obf_basename_h) + '" \n')
                else :
                    out.write(line)
        return obf_dir / obf_basename_c
